...
\marginnote[-2cm]{
    \begin{kaobox}[frametitle=Le procédé de \textsc{Gram}-\textsc{Schmidt}]
        Soit $E$ un espace vectoriel préhilbertien et soit $\mathscr{F} = (e_i)_{i \in I}$ une famille libre dans $E$; il existe une unique famille orthonormale $\mathscr{G} = (\varepsilon_i)_{i \in I}$ telle que 
        \begin{enumerate}
            \item pour tout $k \in \llbracket 1, n \rrbracket$, $$\Vect(e_1, \dots, e_k) = \Vect(\varepsilon_1, \dots, \varepsilon_k)$$
            \item pour tout $k \in \llbracket 1, n \rrbracket, \langle e_k, \varepsilon_k \rangle > 0$.
        \end{enumerate}
