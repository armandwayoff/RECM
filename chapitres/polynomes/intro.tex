...
\marginnote[5.5cm]{
    Nicolas \textsc{Tosel} poly révisions lycée -> CPGE \\
    \note Soient $a, b, c$ des nombres complexes et $P$ le polynôme défini par:
    $$\forall z \in \C,\ P(z) \defeq z^3 + az^2 + bz + c.$$
    On observe que, si $h \in \C$, alors pour tout $z \in \C$
    \begin{align*}
        P(z + h) = z^3 + (3h + a)&z^2 + (3h^2 + 2ha + b)z \\
        & + h^3 + ah^2 + bh + c.
    \end{align*}
    Soit $h \defeq -\frac{a}{3}$. Alors,
