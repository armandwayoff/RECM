...
\todoinline{Préciser le phénomène de Runge}

\todoinline{Utiliser St Cyr 1993}

\todoinline{Ajouter commentaire sur extension à $\interff{a}{b}$.}
...
On pose $\phi : x \mapsto f(x) - P(x) - K \prod_{i=0}^n (x - a_i)$.
\todoinline{Préciser le choix de $K$}
...
\[
\module{f(x) - P(x)} \leq \frac{\norme{f^{(n+1)}}_{infty}}}{(n+1)!} P_a(x).
\]
\end{questions}

Soit $Q$ un polynôme unitaire de degré $n$. On note $T_{n+1}$ le polynôme de Tchebychev d'ordre $n + 1$ et on admet que $t_{n+1} = 2^{-n} t_{n+1}$ est un polynôme unitaire. 
\begin{questions}[resume]
\item Montrer, en utilisant un raisonnement par l'absurde, que $\norme{Q}_\infty \geq \frac{1}{2^n}$.

\item Montrer que, si $\norme{Q}_\infty = \frac{1}{2^n}$, alors $Q = t_n$.
\end{exercice}
\begin{solution}
