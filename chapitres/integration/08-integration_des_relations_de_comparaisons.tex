...
\todoinline{Lier avec la formule de Stirling - À déplacer dans séries numériques ? - Ou alors avec l'intégration terme à terme donc dans le thème précédent qui est un peu vide}
...
\item En appliquant la question précédente aux fonctions en escalier $f(t) = \sum_{k=0}^{+\infty} a_k \indicatrice{[k,k+1[}(t)$ et $g(t) = \sum_{k=0}^{+\infty} b_k \indicatrice{[k, k+1[}(t)$, montrer que si $a_n \sim b_n$ et $\sum a_n$ converge, alors $\sum\limits_{k=n+1}^{+\infty} a_k \sim \sum\limits_{k=n+1}^{+\infty} b_k$.
