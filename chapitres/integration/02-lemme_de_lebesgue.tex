...
\begin{solution}
    \begin{enumerate}
        ...
        &\leqslant         \left|\int_a^b (f(t) - \varphi(t)) \sin(\lambda t) \d t\right| + \left|\int_a^b \varphi(t) \sin(\lambda t) \d t\right|\\
...
\marginnote[-7pt]{\mystartext{\hyperref[thm:prolongementDesDerivees]{Théorème de prolongement des dérivées}} \todoarmand{Mettre en évidence les théorèmes / prop. utilisés ?}}D'après le théorème de prolongement des dérivées, $\varphi_n$ est prolongeable en une fonction de classe $\mathscr{C}^1$ sur $\interfo{0}{1}$.
...
En effet, pour tout $t \in \interoo{0}{1}$, $\e^{2 \i k \pi t} \not= 1$. Ainsi, d'après la somme des termes d'une suite géométrique, 
    \begin{align*}
        ...
        \sum_{k=0}^N \cos(2 k \pi t) &= \cos(N \pi t) \frac{\sin \big((N+1) \pi t \big)}{\sin(\pi t)} \\
