...
\begin{solution}
    \begin{enumerate}
        ...
        \[
        \left| \varphi(t) \int_a^b \sin(\lambda t) \d t\right| \leq \varepsilon.
        \]
...
\begin{theo}{}
Pour tout entier $m \geq 1$, $\zeta(2m) = (-1)^{m-1} (2 \pi)^{2m} \frac{B_{2m}(0)}{2}$.
\end{theo}
...
\marginnote[0cm]{Théorème de prolongement des dérivées \todoarmand{Mettre en évidence les théorèmes / prop. utilisés ?}}
D'après le \textsl{théorème de prolongement des dérivées}, $\varphi_n$ est prolongeable en une fonction de classe $\mathscr{C}^1$ sur $\interfo{0}{1}$.
