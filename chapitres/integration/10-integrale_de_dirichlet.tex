...
\todoinline{Preuve ajoutée, à relire}
...
&= 2 \int_0^\pi \cos (n + 1) t \d t
= \frac{2}{n + 1} \left[\sin(n + 1) t\right]_{0}^\pi
= 0.
\end{align*}

Ainsi, la suite $(I_n)$ est croissante et $I_0 = \frac{\pi}{4}$. Donc, pour tout $n$ entier naturel, $I_n = \frac{\pi}{4}$.

\item La fonction $f$ est continue et dérivable sur $]0; \pi]$. De plus, pour tout réel $x \in ]0; \pi]$,
$f(x) = \frac{x - 2 \sin(x/2)}{2 x \sin(x/2)}$.
...
&= \int_0^\pi \frac{\sin(n + 1/2) t}{t} - \frac{\sin (n + 1/2) t}{2 \sin (t/2)} \d t + \frac{\sin (n + 1/2) t}{2 \sin(t/2)} \d t\\
...
\todoinline{OK, c'est assez long comme ça je pense, on peut s'arrêter là. On peut peut être mettre une référence pour ceux qui veulent aller plus loin}