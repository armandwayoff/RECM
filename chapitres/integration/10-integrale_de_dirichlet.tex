...
\begin{prop}
    L'intégrale de \textsc{Dirichlet} (1829) est l'intégrale de la fonction sinus cardinal sur la demi-droite des réels positifs \todoarmand{changer la formulation}
    \[
...
\begin{demo}[par le théorème des séries alternées]
\begin{enumerate}
...
\]
\marginnote[-7pt]{\theoremeutilise{\hyperref[thm:seriesAlternees]{Théorème des séries alternées}}}Ainsi, d'après le théorème des séries alternées, $\sum u_n$ converge. Attention, la fonction $x \mapsto \int_0^x \frac{\sin t}{t} \d t$ n'est pas monotone et on ne peut donc pas encore conclure.
...
\end{enumerate}
\end{demo}
\begin{demo}[par intégration par parties]\label{demo2existenceIntegraleDirichlet}
...
\todoarmand{Sous section "Régularité du sinus cardinal sur $\R$" déplacée dans la section "Prolongement de fonctions" mais toujours à voir si on la garde}
...
\]
Ainsi, $\lim\limits_{x\to+\infty} F(x) = I$ et
\[
I = \frac{\pi}{2}.
\]
\end{reponses}
...
\begin{exercice}
    Exercice 170 p. 184 \\
    Soient $f(x) = \int_0^\infty \frac{\sin(t)}{t+x} \d t$ et $g(x) = \int_0^\infty \frac{\e^{-xt}}{t^2 + 1} \d t$. 
    \begin{questions}
