...
\begin{marginfigure}[0cm]
    \centering
    \input{illustrations/normesLp_pinfini.pgf}
    \caption{ébauche}
\end{marginfigure}
...
Supposons $p > r$. Soit $\alpha > 0$ et $f(x) = \frac{1}{1+\module{x}^\alpha}$, $x \in \R$. Si on choisit $1/p < \alpha < 1/r$, alors $f \in L^p(\R)$ mais $f \not\in L^r(\R)$, donc $L^p(\R) \not\subset L^r(\R)$. \\
Soient $\alpha > 0$ et $g(x) = \frac{1}{\module{x}^\alpha (1+x^2)}$, $x \in \R$. Si on choisit $1/p <  \alpha < 1/r$, $f \in L^r(\R)$ mais $f \not\in L^p(\R)$, donc $L^r(\R) \not\in L^p(\R)$.