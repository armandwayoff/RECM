...
\todoinline{On ajoute le graphe de la fonction avec l'aire coloriée ?}

%-----------
\subsection{Calculs de l'intégrale}

\todoinline{Je supprimerais la première manière de calculer qui est un peu naïve quand ont a le théorème de convergence dominée. Ça permettrait de gagner du volume.}

\begin{exercice}
\marginnote[0cm]{Source : \cite{maths-france} Planche no 13. Suites et séries d’intégrales}
On note $I = \int_0^{+\infty} \e^{-x^2} \d x$ et on pose $f : t \mapsto \e^{-t^2}$.
...
\end{cases}.
$$
Pour tout réel positif $x$, on pose $f(x) = \e^{-x}$.
\begin{enumerate}
\item Étudier la fonction $f - f_n$ et en déduire que pour tout $x$ réel positif, 
$$|f(x) - f_n(x)| \leqslant \frac{1}{n \e}.$$

\item On pose $I_n= \int_0^{+\infty} f_n(x^2) \d x$. Montrer que $\module{I_n - I} \leq \frac{1}{\e \sqrt{n}} + \int_{\sqrt{n}}^{+\infty} \e^{-x^2} \d x$.
...
\todoinline{Peut-on superposer les graphes de $f_n$ pour $2-3$ valeurs de $n$ et celui de $f$ pour visualiser la convergence simple ?}
