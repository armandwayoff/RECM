...
\begin{marginfigure}
    \centering
    \input{chapitres/integration/illustrations/i_10-planche_de_galton}
    \caption{Planche de \nom{Galton} qui illustre la convergence d'une loi binomiale vers une loi normale}
\end{marginfigure}
...
On dit qu'une variable aléatoire $X$ suit la loi normale centrée réduite, ce que l'on note $X↪N(0,1)$ si elle est continue et admet pour densité : $f(x)=1√2πexp(−x22)$. Une telle variable aléatoire $X$ admet alors une espérance et une variance : $E(X)=0 et V(X)=1$.

Si X suit une loi normale centrée réduite, on a donc $(X∈[a,b])=∫ba1√2πexp(−x22)$. Ce calcul d'intégrale ne peut en général pas être mené de façon exacte, car on ne connait pas de primitive à la densité f. On cherche des valeurs approchées en utilisant une table de la loi normale, ou à l'aide d'un tableur

Cas général

On dit qu'une variable aléatoire X
suit la loi normale de paramètres m∈R et σ2, avec σ>0, ce que l'on note X↪N(m,σ2) si elle est continue et admet pour densité : f(x)=1σ√2πexp(−(x−m)22σ2). Ceci revient à dire que X−mσ suit une loi normale centrée réduite N(0,1). Remarquons que si X suit une loi N(m,σ2), alors X admet alors une espérance et une variance : E(X)=m et V(X)=σ2.
...
On remarquera que la loi normale se concentre essentiellement autour de sa moyenne. Par exemple, si X
suit une loi normale centrée réduite, la probabilité pour que X

soit compris entre -3 et 3 est supérieur à 0,998.
...
Des clous sont plantés sur la partie supérieure de la planche, de telle sorte qu'une bille lâchée sur la planche passe soit à droite soit à gauche pour chaque rangée de clous. Dans la partie inférieure les billes sont rassemblées en fonction du nombre de passages à gauche et à droite qu'elles ont fait.
...
\url{https://fr.wikipedia.org/wiki/Planche_de_Galton}
}
