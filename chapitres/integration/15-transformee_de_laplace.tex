...
\item Si $p > 0$, alors $f(t) = \petito_{+\infty}(1/t^2)$ et $f$ est intégrable sur $\Rpe$.
...
\begin{theo}
\begin{enumerate}
\item Si la fonction $f$ est bornée, alors $\mathscr{L}(f)$ est définie et de classe $\Cont^\infty$ sur $\Rpe$.
...
\begin{demo}
\begin{enumerate}
\item La fonction $f$ est bornée par une constante $M$. On utilise le \theoremeutilise{théorème de dérivation sous le signe intégral}{theo:derivationsoussigneintegrale}. Notons $\fonctionligne[g]{(p, t)}{f(t) \e^{-pt}}$.
\begin{itemize}
\item Pour tout $t \in \Rpe$, la fonction $\fonctionligne[g(\,\cdot\,, t)]{p}{\e^{-p t} f(t)}$ est de classe $\Cont^\infty$ sur $\Rpe$ et
\[
...
\end{itemize}
Ainsi, la transformée de \nom{Laplace} $\mathscr{L}(f)$ est de classe $\Cont^\infty$ sur $\Rpe$.
...
Par hypothèse, $F$ possède une limite finie en $+\infty$. Ainsi, d'après le \theoremeutilise{théorème d'intégration par parties}{theo:ippgeneralisees}, les intégrales $\int_0^{+\infty} \e^{-p t} f(t) \d t$ et $\int_0^{+\infty} \e^{-p t} F(t) \d t$ sont de même nature.
...
\end{itemize}
D'après le \theoremeutilise{théorème de continuité sous le signe intégral}{theo:continuitesoussigneintegral}, la transformée $\mathscr{L}(f)$ est continue sur $\interoo{p_0}{+\infty}$.
\end{enumerate}
\end{demo}
...
\begin{solution}
\begin{reponses}
\item Comme $f$ est continue sur $\Rp$ et possède une limite en $+\infty$, une généralisation classique du \theoremeutilise{théorème des bornes}{theo:bornes} assure que $f$ est bornée sur $\Rp$. Ainsi, d'après le théorème précédent, la transformée $\mathscr{L}(f)$ est bien définie sur $\Rpe$.
...
\item En utilisant la \theoremeutilise{caractérisation séquentielle de la limite}{theo:caracterisationsequentiellelimite},
\[
...
\begin{margintable}
\footnotesize{
\input{chapitres/integration/illustrations/15-transformee_de_laplace-tableau}
}
...
\source{\cite{rouviere2009}}
Soient $f$ une fonction de classe $\Cont^2$ et intégrable sur $\Rp$. Alors,
\[
...
\item À l'aide d'un changement de variable et du \theoremeutilise{théorème de convergence dominée}{theo:convergencedominee}, montrer que
\[
