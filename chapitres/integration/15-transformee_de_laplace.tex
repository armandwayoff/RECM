...
\begin{marginfigure}[0cm]
    \centering
    \input{illustrations/i_transformee_laplace_convolution}
    \caption{Source : \url{https://commons.wikimedia.org/wiki/File:Time_and_frequency_domains.svg?uselang=fr}}
\end{marginfigure}

\begin{theo}
Soient $u$ et $v$ deux fonctions intégrables sur $\R$. Pour tout $x \in \R$, on définit
\begin{equation}\label{defconvolution}
(u \ast v)(x) = \int_{\R^d} u(x-y) v(y) \d y.
\end{equation}
La fonction $u \ast v$ ainsi définie est le \emph{produit de convolution} de $u$ et $v$.
\begin{enumerate}
\item $u \ast v = v \ast u$.

\item $u \ast v$ est une fonction intégrable et
\[
\norm{u \ast v}_{1} \leqslant \norm{u}_{1} \norm{v}_{1}.
    \]
    \end{enumerate}
    \end{theo}

\todoinline{À reprendre sans Fubini ? Est-ce possible ?}
