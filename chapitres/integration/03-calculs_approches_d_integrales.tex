...
\todoinline{Reprendre le graphique.}
\begin{enumerate}
...
\includegraphics[width=0.2\textwidth]{./chapitres/integration/documents/figures/rectangles_medians.jpg}
...
\[
\module{\int_{\interff{a}{b}} f - I_p^\mathrm{m}(f)} \leq \frac{M_2 (b-a)^3}{24 p^2}.
\]
...
\begin{elem_sol}
On suppose que $f$ est une fonction de classe $\mathscr{C}^2$ et on note $M_2 = \sup_{\interff{a}{b}} \module{f''}$. Pour tout $i \in \interent{0}{p-1}$, on note $\phi_i$ l'approximation affine sur $\interff{x_i}{x_{i+1}}$ de $f$ et $g_i = f - \phi_i$.
...
\[
\module{\int_{\interff{a}{b}} f - I_p^\mathrm{t}(f)} \leq \frac{M_2 (b-a)^3}{12 p^2}.
\]
...
\begin{remarque}
Lorsque $f$ est de classe $\mathscr{C}^2$ et convexe, alors $f'' \geq 0$ et, pour tout entier naturel $p$, $\int_{\interff{a}{b}} f \leq I_p^\mathrm{t}(f)$. On obtient ainsi une valeur approchée par excès de l'intégrale.
\end{remarque}
...
\todoinline{Reprendre la section suivante.}
...
On note $(L_{i,0},\ldots,L_{i,p})$ la famille des \hyperref[sec:polynomes_de_lagrange]{polynômes d'interpolation de \textsc{Lagrange}} aux points $(y_{i,0},\ldots,y_{i,n})$. Pour tout entier $i \in \interent{0}{n}$, on note $\lambda_i = \int_a^b L_i(t) \d t$. La méthode de Newton-Cotes approche l'intégrale $\int_a^b f(t) \d t$ par
\[
