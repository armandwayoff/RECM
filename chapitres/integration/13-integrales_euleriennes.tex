...
\subsection{Fonction Gamma d'\textsc{Euler}}\label{subsec:FonctionGammaEuler}
...
\todoinline{Supprimer la référence à l'IPP ici pour lier vers une annexe.}
...
%------------
\subsection{Caractérisation de $\Gamma$}
...
\todoinline{J'avais pas vu en détail mais normalement c'est $p-1$ et $q-1$ pour avoir la fonction bêta. On a donc deux solutions : on laisse comme ça en mettant une remarque, on modifie tout (puis on relit !) Tu en penses quoi ?}
...
\item Pour tout $(p, q) \in \N \times \Ne$, montrer que $B(p, q) = \frac{q}{p + 1} B(p+1, q-1)$.
...
\todoinline{J'ai arnaqué et je n'ai pas vérifié la log-concavité. Par contre, Centrale 1 - MP - 2009 montre ce même résultat (et plus). Ce serait peut être bien de l'implémenter ici ?}

De plus, on peut étendre la fonction bêta à $\Rpe$ en posant
\[
B(x, y) = \int_0^1 t^{x-1} (1 - t)^{y - 1} \d t.
\]

On obtient alors avec une intégration par parties la relation
\[
B(x + 1, y) = \frac{y}{x + y} B(x, y).
\]

Posons ainsi $f : t \mapsto \Gamma(y + t) B(t, y)$. Alors,
\begin{align*}
f(1)
&= \Gamma(y + 1) B(1, y)
= y \Gamma(y) B(0, y - 1) = \Gamma(y)\\
f(t + 1)
&= \Gamma(y + t + 1) B(t + 1, y)
= y \Gamma(y + t) B(t, y)
= y f(t).
...
\todoinline{Ajouter un lien vers le théorème de caractérisation.}

De plus, on montre comme pour la fonction $\Gamma$ que $t \mapsto \ln B(t, y)$ est convexe. Ainsi, $\frac{f}{\Gamma(y)}$ satisfait la caractérisation de la fonction $\Gamma$ et
\[
\forall,\, (x, y) \in (\Rpe)^2,\,
