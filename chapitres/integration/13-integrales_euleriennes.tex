...
\item Les fonctions $t \mapsto \e^{-t}$ et $t \mapsto t^x$ sont de classe $\mathscr{C}^1$ sur $\R_+^\star$. D'après les croissances comparées, $\lim_{t\to0} t^x \e^{-t} = \lim_{t\to+\infty} t^x \e^{-t} = 0$. \item \marginnote[-7pt]{\hyperref[theo:ippgeneralisees]{\theoremeutilise{Intégration par parties généralisée}}}D'après la formule d'intégration par parties généralisée,
\[
